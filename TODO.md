...
- [ ] All money-changing requests use idempotency keys — checkout and refunds do (refunds pass `services.IdempotencyKey` to the provider call at `handlers/payments.go:752`); payout creation is now reachable (`/sell/payouts/request`), but `store.CreateRefund`/`store.CreatePayout` still take no local dedup key of their own, so a double-submitted form is only guarded by user-visible PRG redirect, not a DB-level unique constraint
