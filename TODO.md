...
- [ ] All money-changing requests use idempotency keys — checkout and refunds do (refunds pass `services.IdempotencyKey` to the provider call at `handlers/payments.go:752`); payout *creation* has no admin-reachable flow yet (only review/approval of already-created rows), and `store.CreateRefund`/`store.CreatePayout` still take no local dedup key, so a double-submitted admin form is only guarded upstream at the provider, not in the local DB row
